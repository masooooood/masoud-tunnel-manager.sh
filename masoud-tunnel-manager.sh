...
# ==========================================================
# Masoud Tunnel Manager (v3 - Remote Bootstrap + Service Manager)
# Ubuntu 22.04
#
# Features:
...
write_env() {
  # KEY=VALUE each on its own line
...
# -------- Remote Bootstrap (no manual login required) --------
# Uses ssh-copy-id (will ask password once if key not installed)
remote_bootstrap() {
...
remote_remove_agent() {
  local host="$1" user="$2" port="$3" key="$4" name="$5"
  local agent="masoud-tun-agent-${name}"
  ssh -i "$key" -p "$port" -o StrictHostKeyChecking=no "${user}@${host}" bash -s -- "$agent" <<'EOS'
set -euo pipefail
AGENT="$1"
systemctl disable --now "${AGENT}.service" 2>/dev/null || true
rm -f "/etc/systemd/system/${AGENT}.service"
rm -f "/usr/local/bin/${AGENT}.sh"
rm -f "/etc/masoud-tunnels/${AGENT}.env"
systemctl daemon-reload
EOS
}
